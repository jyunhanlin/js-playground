...
- **design-pattern/** — GoF patterns organized by category (`behavioral/`, `creational/`, `structural/`). `behavioral/` and `structural/` are markdown notes (concept + JS examples); `creational/` is still JS source files. One JS file remains in `behavioral/` (PubSub.js). `behavioral/README.md` is the cross-pattern overview; each behavioral note has a `**Reified:**` line under `Core Roles` that feeds it — keep the two in sync. `structural/README.md` is the same kind of overview (compares wrappers by what they wrap, interface change, and who builds the wrap).
