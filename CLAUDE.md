...
- **design-pattern/** — GoF patterns organized by category (`behavioral/`, `creational/`, `structural/`). All three are markdown notes (concept + JS examples); one JS file remains in `behavioral/` (PubSub.js). Each category has a `README.md` cross-pattern overview: `behavioral/` compares what each pattern reifies (each note has a `**Reified:**` line under `Core Roles` that feeds it — keep the two in sync); `structural/` compares wrappers by what they wrap, interface change, and who builds the wrap; `creational/` compares who decides the concrete class, when, and how many instances.
...
Solo playground repo — commit and push directly to `main`. Never open PRs or delete remote branches
unless I ask. (`gh pr create` is hook-enforced here.)
