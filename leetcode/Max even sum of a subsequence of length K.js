// ref: https://github.com/ReginaF2012/JavaScript_MaxHeap/blob/main/maxHeap.js
class MaxHeap {
  constructor() {
    this.values = [];
  }
...
const maxSum = (arr, k) => {
  ...
  arr.forEach((a) => {
    if (a % 2) oddMaxHeap.add(a);
    else evenMaxHeap.add(a);
  });

  let result = 0;

  while (k && oddMaxHeap.size() && evenMaxHeap.size()) {
    if (oddMaxHeap.peek() < evenMaxHeap.peek() || k < 2) {
      result += evenMaxHeap.extractMax();
      k -= 1;
    } else {
      if (oddMaxHeap.size() > 1) {
        const oddMax = oddMaxHeap.extractMax();
        const oddSecMax = oddMaxHeap.extractMax();

        const evenMax = evenMaxHeap.extractMax();
        const evenSecMax = evenMaxHeap.extractMax();

        if (oddMax + oddSecMax > evenMax + evenSecMax) {
          result = result + oddMax + oddSecMax;
          k -= 2;

          evenMaxHeap.add(evenMax);
          evenMaxHeap.add(evenSecMax);
        } else {
          result += evenMax;
          k -= 1;
          oddMaxHeap.add(oddMax);
          oddMaxHeap.add(oddSecMax);
          evenMaxHeap.add(evenSecMax);
        }
      } else {
        result += evenMaxHeap.extractMax();
  ...
  return result === 0 || k !== 0 ? -1 : result;
};

console.log(maxSum([4, 2, 6, 7, 8], 3) === 18);
console.log(maxSum([5, 5, 2, 4, 3], 3) === 14);
